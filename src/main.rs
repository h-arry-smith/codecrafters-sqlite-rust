...
fn parse_db_header<R: Read + ByteReader>(reader: &mut R) -> DbHeader {
    ...
    // TODO: See spec regarding this mode, and verify invariants regarding its use hold true after
    ...
    DbHeader {
        ...
        user_version,
        incremental_vacuum_mode: largest_root_btree_page_number != 0,
        application_id,
        version_valid_for,
...
fn handle_dot_command(command: &str, args: &[String]) -> Result<()> {
    match command {
        "dbinfo" => {
            ...
            let header = parse_db_header(&mut file);
